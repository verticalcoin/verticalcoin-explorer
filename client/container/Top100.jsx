...
class Top100 extends Component {
  ...
  render() {
    let supply = this.props.coin.supply;
    if (supply <= 0 && !!this.state.wallets.length) {
      supply = this.state.wallets.reduce((a, v) => {
        if (!isNaN(a.value)) {
          return a.value;
        }
        return a + v.value;
      ...
      <div>
        ...
        <Table
          ...
          data={ this.state.wallets.map((wallet, idx) => ({
            ...
            index: idx + 1,
            percent: numeral((wallet.value / supply) * 100.0).format('0,0.00'),
